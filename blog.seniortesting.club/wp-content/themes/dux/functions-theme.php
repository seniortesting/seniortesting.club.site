...
function _get_post_thumbnail($size = 'thumbnail', $class = 'thumb') {
	...
	if($r_src){
		// 在“列表”-》“文章缩略图异步加载 (v1.4+)”中不要勾选该选项
		if(_hui('thumbnail_src')){
			$random = mt_rand(1, 11);//数字1-9的图片
    		...
    		return sprintf('<img data-src="%s" alt="%s" src="%s" class="thumb">', $r_src, $post->post_title._get_delimiter().get_bloginfo('name'), get_stylesheet_directory_uri().'/img/random/'.$random.'.jpg');
		}else{
