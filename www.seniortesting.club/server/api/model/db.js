...
pool.on('acquire', function (connection) {
  log.info(`Connection Thread ${connection.threadId} acquired`)
})
pool.on('enqueue', function () {
  log.info('Waiting for available connection slot')
})
pool.on('release', function (connection) {
  log.info(`Connection Thread ${connection.threadId} released`)
})
...
const exec = function (query = '', bindParameters = [], callback) {
  pool.getConnection((err, connection) => {
    if (err) {
      log.error(`CONNECT Thread ${connection.threadId} , ERROR: ${err.message}`)
    } else {
      if ((!bindParameters) || bindParameters.length === 0) {
        bindParameters = []
      }
      log.info(`[SQL] Query: ${query}`)
      log.info(`[SQL] parameter: ${bindParameters.toString()}`)
