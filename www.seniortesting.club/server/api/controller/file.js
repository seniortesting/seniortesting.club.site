...
const sql = {
  addFileSql: `
   INSERT INTO  sys_file_upload(id,token,original_name,file_name,file_path,url,create_time,create_by,update_by,update_time)
  VALUES (?,?,?,?,?,?,
  NOW(),?,?,NOW()
  )
  `,
  deleteFileSql: `
   SELECT id, file_name, file_path from sys_file_upload WHERE token=? AND url =?;
   DELETE FROM sys_file_upload WHERE token=? AND url =?;
  `,
  getFileListSql: `
  SELECT id, original_name, file_name, file_path, url, update_time FROM sys_file_upload WHERE token=?
  `
}
const file = {
  upload: (req, res, next) => {
    ...
    try {
      ...
      req.files.forEach((uploadFile) => {
        ...
        // 插入数据库
        const params = [insertId, token, uploadOriginalName, uploadFileName, uploadPath, uploadUrl, userId, userId]
  ...
  getFileListByToken: (req, res) => {
    ...
    try {
      ...
      // 插入数据库
      const params = [token]
      db.exec(sql.getFileListSql, params, (error, results, fields) => {
        if (error) {
          result.code = 501
          result.message = JSON.stringify(error)
          return res.json(result)
