    ...
    def get_video_url(self, source_url, try_num=3):
        ...
        if video_id != '':
            ...
            if content_dict.get('code') != 0:
                ...
                # 选择高分辨率的视频
                videolist = content_dict['data']['video_list']
                maxsize = 0
                maxkey = ''
                for k, v in videolist.items():
                    size = v.get('size')
                    if size > maxsize:
                        maxsize = size
                        maxkey = k
                if maxkey is not None:
                    main_url = videolist.get(maxkey)['main_url']
                    video_url = base64.b64decode(main_url).decode()
                    return video_url
        else:
            print('解析地址的video_id出错: {}'.format(source_url))


def download_file(name, url, dir='D:\\自媒体\\搞笑片段\\'):
    print('开始下载')
        ...
        for chunk in r.iter_content(chunk_size=512):
            if chunk:
                ...
                if time.time() - last_time > 2:
                    ...
                    count_tmp = count
                    print('{}下载{:.2f}%---{:.2f}M/s'.format(dir + name, p, speed))
                    last_time = time.time()
        print('下载完成')
